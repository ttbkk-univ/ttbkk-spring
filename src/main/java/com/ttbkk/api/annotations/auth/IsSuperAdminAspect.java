...
@Aspect     // AOP Aspect
@Component
public class IsSuperAdminAspect extends BaseAuthCheckAspect {
    /**
     * 클래스 생성자.
     ...
     */
    public IsSuperAdminAspect(HttpServletRequest httpServletRequest, JWTService jwtService, UserService userService) {
     ...
     */
    @Around("@annotation(com.ttbkk.api.annotations.auth.IsSuperAdmin)") // 어노테이션과 Aspect 연결
    public Object check(final ProceedingJoinPoint joinPoint) throws Throwable {
        User superAdmin = this.getUser();
        // TODO user role 체크하는 로직
        ArrayList<String> targets = new ArrayList<>(Arrays.asList("SUPER_ADMIN"));
//        targets.contains(user.role);
        return joinPoint.proceed(new Object[]{superAdmin});
    }
}
